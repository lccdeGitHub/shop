...
2.技术选型
2.1.前台应用：Vue技术栈
2.2.与后台交互axios，测试接口，模拟数据mock.js
     基于promise实现的，ES6模块，Babel，wepack，vue-cil脚手架
2.3.Css预编辑器stylus
3.API接口：一个接口是url+请求方式+请求数据格式+返回数据格式
           接口文档内有接口内的所有信息
           通不通，是不是和文档一致。
4.你从项目中学到了什么~~
## 2. 开启项目开发
    ...
    安装所有依赖/指定依赖
    开发环境运行：两种方法，开发环境和生产环境运行。必须先打包，生成环境打包后会生成dist文件夹 npm run build
